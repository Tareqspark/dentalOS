...
/** Maps landing-page feature cards to their dedicated module pages. */
const slugByTitle: Record<string, string> = {
  "AI Receptionist": "ai-receptionist",
  "Smart Scheduling": "smart-scheduling",
  "Electronic Dental Records": "dental-records",
  "Insurance Automation": "insurance-automation",
  "Billing & Payments": "billing-payments",
  "Patient Portal": "patient-portal",
  "AI Clinical Notes": "ai-clinical-notes",
    ...
    <section className="py-24 md:py-32" id="features">
      <Container>
        <SectionHeading
          ...
          subtitle="Every module is built in — not bolted on. One login, one patient record, one monthly bill. Click any module to explore it in depth."
        />
        ...
        <Stagger className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {features.map(({ icon: Icon, title, description, ai }) => (
            <StaggerItem key={title} className="h-full">
              <Link href={`/features/${slugByTitle[title] ?? ""}`} className="block h-full">
